...
An example was the Coronavirus Infection Survey (CIS)~\citep{cisMethodsONS,CIS}.
CIS was a gold-standard survey, inviting households selected at random from the UK population to partake and testing individuals in them on a regular schedule.
...
This design required a level of resources rarely available to infectious disease surveillance systems but provided data unimpacted by individuals' ability or desire to test, and independent of the severity of their disease.
...
These models have weak assumptions over the course that the pandemic takes, reducing the risk of introducing bias through incorrect assumptions, but cannot capture the underlying dynamics of transmission.
...
This type of models provide deeper insights into transmission dynamics, such as the rate at which individuals are infected and the susceptibility of different demographic groups.
In this work, we develop both phenomenological and mechanistic models to analyse the CIS data.

During the pandemic, various methods were proposed to estimate incidence and transmission from CIS data, or the other gold-standard survey in the UK, the REal-time Assessment of Community Transmission (REACT)~\citep{rileyResurgence,rileyREACT}.
Some of these~\citep{daviesAssociation,ironsEstimating,knockKey,pooleyEstimation} incorporated mechanistic models with age structure, as our mechanistic model does, but used a coarse approximation of the distribution of the duration of RT-PCR positivity and incorporated other data sources.
One study used granular geographic scale and a complex observational model but a simpler transmission model without incorporating age; their primary aim is to ``debias'' the pillar 2 data~\citep{nicholsonImproving}.
Others take a phenomenological approach~\citep{abbottCISincidence,colmanAscertainment,mccabeCISincidence,ealesAppropriately} but do not fully propagate the uncertainty in CIS estimates of prevalence, relying on only summary statistics

Here, we advance previous work by fitting solely to CIS data, providing a more accurate representation of the duration of RT-PCR positivity, using a duration distribution based on a representative sample, and fully propagating the survey uncertainty.
...
We show that CIS data can be used not only to estimate prevalence but also to derive accurate estimates of incidence and transmission. These findings underscore the potential of prevalence surveys like CIS to serve as a cornerstone for pandemic surveillance and response.
...
Susceptible individuals can become exposed by their interaction with infectious individuals, determined by contact rates stratified by age.
These rates combine pre-pandemic surveys with real-time data on time-use.
...
The lack of uncertainty for the date of the peak in the mechanistic model is because the model structure only allows step changes in growth rates to occur on Mondays, leading to peaks also occurring on Mondays.
\begin{figure}%[tbhp]
\centering
...
There is similar agreement between the models when considering incidence estimates for each region (\cref{fig:incidence-strat}(A)).
...
The largest disagreement between the models is that the phenomenological model estimates a period of increasing incidence in January for England (\cref{fig:incidence-Eng}), although the credible intervals overlap for the majority of the period.
